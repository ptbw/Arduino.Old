...
Adafruit_IO_Feed testFeed = aio.getFeed("frontdoor");
...
void loop() {
  ...
  String colour = "#" + String(count,HEX);  
  if (testFeed.send(colour.c_str())) {
    Serial.print(F("Wrote value to feed: ")); Serial.println(colour);
  }
  else {
...
//  }
  ...
  Serial.println(F("Waiting 2 seconds and then writing a new feed value."));
  delay(2000);
}
